...
RegisterCommand('admingivecar', function(source, args)
    ...
    if not targetId or not vehicle then return end
    ...
    MySQL.insert.await('INSERT INTO player_vehicles (citizenid, plate, vin, vehicle, hash, state, garage) VALUES (?, ?, ?, ?, ?, 1, "legionsquare")', {
        identifier, plate, vin, vehicle, hash
    })
    ...
    TriggerClientEvent('adv_vehicles:client:AdminSpawnCar', targetId, vehicle, plate)
end, true)
