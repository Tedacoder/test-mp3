...
function renderVehicles(vehicles) {
    ...
    vehicles.forEach(veh => {
        ...
        if (veh.state === 1) {
            ...
            // Encode the string safely for HTML injection
            let safeCoords = encodeURIComponent(veh.coords);
            btnHtml = `<button class="btn" onclick="locateVehicle('${veh.plate}', '${safeCoords}')">Locate</button>`;
        } else if (veh.state === 2) {
...
function locateVehicle(plate, safeCoords) {
    document.getElementById('garage-container').style.display = 'none';
    let coords = decodeURIComponent(safeCoords);
